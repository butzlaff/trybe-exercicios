...
const handleCompareAnswers = (rightAnswer, studentAnswer) => {
...
const handleCheckPoints = (rightAnswers, studentAnswers, compareAnswers) => {
  let totalPoints = 0;
  for (const key in rightAnswers) {
    const total = compareAnswers(rightAnswers[key], studentAnswers[key]);
    totalPoints += total; 
    }
  return totalPoints;
};

console.log(handleCheckPoints(RIGHT_ANSWERS, STUDENT_ANSWERS, handleCompareAnswers))